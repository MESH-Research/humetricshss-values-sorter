# frozen_string_literal: true

require "rails_helper"

RSpec.describe "Deleting an activity" do
  let(:user) { nil }
  let(:library) { create(:user).library }
  let(:activity) { create(:activity, library: library, name: "the activity") }

  before do
    sign_in user if user.present?
    visit edit_library_activity_path(library, activity)
  end

  it_behaves_like "it shows a 404 page"

  context "when a user is logged in" do
    let(:user) { create(:user) }

    it_behaves_like "it shows a 404 page"
  end

  context "when the library owner is logged in" do
    let(:user) { library.owner }

    it "allows the user to delete the activity" do
      expect(page).to have_content "Edit Activity"

      expect do
        accept_prompt do
          click_link "Delete Activity", href: library_activity_path(library, activity)
        end

        expect(page).to have_content "Manage My Library"
      end.to change { library.reload.activities.size }.from(1).to(0)

      expect(page).not_to have_content "the activity"
    end
  end
end
