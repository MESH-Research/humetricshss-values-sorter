...
RSpec.describe Activity, type: :model do
  ...
  let(:name) { "Putting on an exhibit" }
  let(:icon_class) { "fas fa-chalkboard-teacher" }

  describe "associations" do
    describe "#advice" do
      subject(:advice) { instance.advice }

      let!(:instance_advice) { create_list(:advice, 3, library: library, activity: instance) }
      let!(:other_advice) { create(:advice) }

      it "contains only the Activity's associated advice" do
        expect(advice).to contain_exactly(*instance_advice)
      end

      it "keeps the Activity from being destroyed when nonempty" do
